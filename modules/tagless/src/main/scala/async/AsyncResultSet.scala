...
/**
 * Implementation of `JdbcResultSet` that wraps a `java.sql.ResultSet` and lifts its operations
 * into blocking operations on `RTS[F]`, logged at `TRACE` level on `log`.
 */
@SuppressWarnings(Array("org.wartremover.warts.Overloading"))
class AsyncResultSet[F[_]: Sync](val value: ResultSet, val rts: RTS[F]) extends JdbcResultSet[F] {

  def absolute(a: Int): F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"absolute($a)")
      value.absolute(a)
    }

  val afterLast: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "afterLast()")
      value.afterLast()
    }

  val beforeFirst: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "beforeFirst()")
      value.beforeFirst()
    }

  val cancelRowUpdates: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "cancelRowUpdates()")
  ...
  val deleteRow: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "deleteRow()")
      value.deleteRow()
    }

  def findColumn(a: String): F[Int] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"findColumn($a)")
      value.findColumn(a)
    }

  val first: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "first()")
      value.first()
    }
  ...
  def getAsciiStream(a: Int): F[InputStream] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"getAsciiStream($a)")
  ...
  def getBinaryStream(a: Int): F[InputStream] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"getBinaryStream($a)")
  ...
  val getConcurrency: F[Int] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "getConcurrency()")
      value.getConcurrency()
    }

  val getCursorName: F[String] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "getCursorName()")
      value.getCursorName()
    }
  ...
  val getRow: F[Int] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "getRow()")
      value.getRow()
    }
  ...
  val getStatement: F[Statement] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "getStatement()")
      value.getStatement()
    }
  ...
  val getType: F[Int] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "getType()")
      value.getType()
    }
  ...
  def getUnicodeStream(a: Int): F[InputStream] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"getUnicodeStream($a)")
  ...
  val insertRow: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "insertRow()")
      value.insertRow()
    }

  val isAfterLast: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "isAfterLast()")
      value.isAfterLast()
    }

  val isBeforeFirst: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "isBeforeFirst()")
      value.isBeforeFirst()
    }
  ...
  val isFirst: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "isFirst()")
      value.isFirst()
    }

  val isLast: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "isLast()")
      value.isLast()
    }
  ...
  val last: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "last()")
      value.last()
    }

  val moveToCurrentRow: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "moveToCurrentRow()")
      value.moveToCurrentRow()
    }

  val moveToInsertRow: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "moveToInsertRow()")
      value.moveToInsertRow()
    }

  val next: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "next()")
      value.next()
    }

  val previous: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "previous()")
      value.previous()
    }

  val refreshRow: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "refreshRow()")
      value.refreshRow()
    }

  def relative(a: Int): F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"relative($a)")
      value.relative(a)
    }

  val rowDeleted: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "rowDeleted()")
      value.rowDeleted()
    }

  val rowInserted: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "rowInserted()")
      value.rowInserted()
    }

  val rowUpdated: F[Boolean] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "rowUpdated()")
      value.rowUpdated()
    }
  ...
  def updateArray(a: Int, b: SqlArray): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateArray($a, $b)")
  ...
  def updateAsciiStream(a: Int, b: InputStream): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateAsciiStream($a, $b)")
      value.updateAsciiStream(a, b)
    }

  def updateAsciiStream(a: Int, b: InputStream, c: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateAsciiStream($a, $b, $c)")
  ...
  def updateBigDecimal(a: Int, b: BigDecimal): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBigDecimal($a, $b)")
  ...
  def updateBinaryStream(a: Int, b: InputStream): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBinaryStream($a, $b)")
      value.updateBinaryStream(a, b)
    }

  def updateBinaryStream(a: Int, b: InputStream, c: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBinaryStream($a, $b, $c)")
  ...
  def updateBlob(a: Int, b: Blob): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBlob($a, $b)")
  ...
  def updateBlob(a: Int, b: InputStream, c: Long): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBlob($a, $b, $c)")
  ...
  def updateBoolean(a: Int, b: Boolean): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBoolean($a, $b)")
  ...
  def updateByte(a: Int, b: Byte): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateByte($a, $b)")
  ...
  def updateBytes(a: Int, b: Array[Byte]): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateBytes($a, $b)")
  ...
  def updateCharacterStream(a: Int, b: Reader): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateCharacterStream($a, $b)")
      value.updateCharacterStream(a, b)
    }

  def updateCharacterStream(a: Int, b: Reader, c: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateCharacterStream($a, $b, $c)")
  ...
  def updateClob(a: Int, b: Clob): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateClob($a, $b)")
  ...
  def updateClob(a: Int, b: Reader, c: Long): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateClob($a, $b, $c)")
  ...
  def updateDate(a: Int, b: Date): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateDate($a, $b)")
  ...
  def updateDouble(a: Int, b: Double): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateDouble($a, $b)")
  ...
  def updateFloat(a: Int, b: Float): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateFloat($a, $b)")
  ...
  def updateInt(a: Int, b: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateInt($a, $b)")
      value.updateInt(a, b)
    }
  ...
  def updateLong(a: Int, b: Long): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateLong($a, $b)")
  ...
  def updateNCharacterStream(a: Int, b: Reader): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateNCharacterStream($a, $b)")
      value.updateNCharacterStream(a, b)
    }

  def updateNCharacterStream(a: Int, b: Reader, c: Long): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateNCharacterStream($a, $b, $c)")
  ...
  def updateNClob(a: Int, b: NClob): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateNClob($a, $b)")
  ...
  def updateNClob(a: Int, b: Reader, c: Long): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateNClob($a, $b, $c)")
  ...
  def updateNString(a: Int, b: String): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateNString($a, $b)")
  ...
  def updateNull(a: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateNull($a)")
      value.updateNull(a)
    }
  ...
  def updateObject(a: Int, b: AnyRef): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateObject($a, $b)")
      value.updateObject(a, b)
    }

  def updateObject(a: Int, b: AnyRef, c: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateObject($a, $b, $c)")
  ...
  def updateObject(a: Int, b: AnyRef, c: SQLType, d: Int): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateObject($a, $b, $c, $d)")
  ...
  def updateRef(a: Int, b: Ref): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateRef($a, $b)")
      value.updateRef(a, b)
    }
  ...
  val updateRow: F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, "updateRow()")
      value.updateRow()
    }

  def updateRowId(a: Int, b: RowId): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateRowId($a, $b)")
  ...
  def updateSQLXML(a: Int, b: SQLXML): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateSQLXML($a, $b)")
  ...
  def updateShort(a: Int, b: Short): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateShort($a, $b)")
  ...
  def updateString(a: Int, b: String): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateString($a, $b)")
  ...
  def updateTime(a: Int, b: Time): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateTime($a, $b)")
  ...
  def updateTimestamp(a: Int, b: Timestamp): F[Unit] =
    rts.newBlockingPrimitive {
      rts.log.unsafe.trace(value, s"updateTimestamp($a, $b)")
