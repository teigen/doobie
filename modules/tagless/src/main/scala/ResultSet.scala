...
import doobie.Read
import doobie.tagless.jdbc.JdbcResultSet
...
trait ResultSet[F[_]] {
  ...
  def next[A](implicit ra: Read[A]): F[Option[A]]

  /** Move to the next row (if any) and read into `A`, raising an error if more rows exist. */
  def option[A: Read](implicit ev: MonadError[F, Throwable]): F[Option[A]]

  /** Move to the next row (if any) and read into `A`, raising an error if more rows exist. */
  def unique[A: Read](implicit ev: MonadError[F, Throwable]): F[A]

  /**
   * Accumulate up to `chunkSize` rows, interpreting rows as type `A`, into a collection `C`,
   * using `CanBuildFrom`, which is very fast.
   */
  def chunk[C[_], A: Read](chunkSize: Int)(implicit cbf: CanBuildFrom[Nothing, A, C[A]]): F[C[A]]
   ...
   * when possible.
   */
  def chunkMapA[C[_]: Alternative, A: Read, B](chunkSize: Int, f: A => B): F[C[B]]

}

object ResultSet {

  def async[F[_]: Sync](jdbc0: AsyncResultSet[F], interp: Interpreter[F]): ResultSet[F] =
    new ResultSet[F] {
